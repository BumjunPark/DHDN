function [curr_dat_sz, curr_lab_sz] = store2hdf5(filename, data, labels, create, startloc, chunksz, dtype)  
  ...
  if create
    ...
    h5create(filename, '/data', [dat_dims(1:end-1) Inf], 'Datatype', dtype, 'ChunkSize', [dat_dims(1:end-1) chunksz]); % width, height, channels, number 
    h5create(filename, '/label', [lab_dims(1:end-1) Inf], 'Datatype', dtype, 'ChunkSize', [lab_dims(1:end-1) chunksz]); % width, height, channels, number 
  ...
  if ~isempty(data)
      ...
      if strcmp(dtype, 'uint8')
          h5write(filename, '/data', uint8(data), startloc.dat, size(data));
          h5write(filename, '/label', uint8(labels), startloc.lab, size(labels));  
      end
  end
